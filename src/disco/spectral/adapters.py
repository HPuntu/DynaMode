...
from src.models.conditioned_freq_scale import ConditionedFreqScaleLookup
...
class SpectralAdapter:
    ...
    This class now owns the shared spectral transform / normalisation /
    conditioned-scale lookup utilities used across training, validation,
    inference, and the spectral models themselves.

    Legacy smoothing / shelf / damping knobs are intentionally stripped out.
    The constructor still accepts and ignores unknown legacy kwargs so stale
    call sites fail less violently while the repo finishes converging.
    """
