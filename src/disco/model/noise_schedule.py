...
The training code diffuses raw spectral coefficients, but most spectral models
normalise those coefficients internally by ``freq_scale``.  This module keeps
that distinction explicit: presets can be expressed in raw coefficient space or
in model-normalised space and are resolved to the raw ``aniso_weights`` tensor
expected by :class:`src.models.diffusion.SpectralDiffusion`.
"""
...
from src.models.diffusion import make_aniso_weights
...
class ResolvedNoiseSchedule:
    """Resolved diffusion schedule ready for ``SpectralDiffusion``."""
...
def _normalize_raw_power(weights: torch.Tensor) -> torch.Tensor:
    weights = weights.float().flatten().clamp(min=1e-12)
    return weights / weights.pow(2).mean().sqrt().clamp(min=1e-12)
...
def _default_target_crossings(num_groups: int, *, pressure: bool = False) -> list[int]:
    ...
    # Generic fallback: monotone low-to-high over a similar time span.
...
def _band_model_snr_curves(
    ...
    *,
    ...
    model_noise = (raw_weights.to(device).float() / freq_scales.to(device).float()).clamp(min=1e-12)
    return _band_snr_curves(
        model_noise,
        channels=channels,
...
def resolve_noise_schedule(
    *,
    ...
    if freq_scales is None and preset in {"v12_current", "legacy_high_k"}:
        raise ValueError(
