...
from disco.spectral.adapters import DCT, compute_frequency_stats
...
from disco.utils import maintain_cache_size, no_spill_stratified_split
...
from src.models.registry import (
    ...
    HierarchicalSpectralConfig, CascadeSpectralConfig,
...
from src.models.mask import frequency_channel_mask, group_order, sample_frequency_mask_selection
...
def _is_hierarchical_frequency_model(inner_model) -> bool:
    return bool(
        hasattr(inner_model, "band_edges")
        and hasattr(inner_model, "num_frequency_groups")
        and hasattr(inner_model, "frequency_ordering")
    )


def _frequency_group_mask(
    *,
    top_k_freqs,
    channels_per_mode,
    band_edges,
    ...
    return flat * full_mask.to(device=device, dtype=dtype)


@torch.no_grad()
def hierarchical_denoise_ode(
    diffusion,
    model,
    input_noise,
    native_coords,
    ...
    *,
    ...
    """Low-to-high frequency-group sampler for HierarchicalSpectralDiffusion."""
    B, L, D = input_noise.shape
    device = input_noise.device
    dtype = input_noise.dtype
    full_mask = spectral_mask.reshape(B, L, D).to(device=device, dtype=dtype)
    prior_noise = input_noise * full_mask
    x_clean = torch.zeros_like(prior_noise)
    order = group_order(len(tuple(band_edges)) - 1, ordering)
    times = torch.linspace(diffusion.T - 1, 0, num_steps, dtype=torch.long, device=device)

    base_model = getattr(model, "base_model", model)
    inner = getattr(base_model, "model", base_model)

    for stage_idx, target_group in enumerate(order):
        observed_groups = tuple(order[:stage_idx])
        target_groups = (int(target_group),)
        observed_mask = _frequency_group_mask(
        ...
        target_mask = _frequency_group_mask(
            top_k_freqs=top_k_freqs,
            channels_per_mode=channels_per_mode,
            band_edges=band_edges,
        ...
        x_target_t = diffusion.sample_initial_noise((B, L, D), device=device).to(dtype=dtype) * target_mask

        for i, t_current in enumerate(times):
            t_batch = torch.full((B,), t_current, device=device, dtype=torch.long)
            sqrt_ab = torch.sqrt(diffusion.alpha_bar[t_current]).to(device=device, dtype=dtype)
            sqrt_one_minus = torch.sqrt(1.0 - diffusion.alpha_bar[t_current]).to(device=device, dtype=dtype)

            observed_xt = (sqrt_ab * x_clean + sqrt_one_minus * prior_noise) * observed_mask
            model_input = observed_xt + x_target_t
            pred = model(
                model_input,
                t_batch,
                temps,
                ...
                res_type=res_type,
                dssp=dssp,
                freq_keep_groups=observed_groups,
                freq_target_groups=target_groups,
            )
            x0_pred, eps_pred = diffusion.extract_x0_eps_from_prediction(
                pred, model_input, t_batch, model.prediction_target
            )

            if model.prediction_target != "x_0" and hasattr(inner, "apply_rmsf_prior_gain"):
                x0_pred = inner.apply_rmsf_prior_gain(x0_pred, rmsf_prior, mask=mask)
                eps_pred = diffusion.prediction_from_x0(
                    x0_pred, model_input, t_batch, "noise"
                )

            if i == len(times) - 1:
                x_target_t = x0_pred * target_mask
                break

            t_next = times[i + 1]
            sqrt_ab_next = torch.sqrt(diffusion.alpha_bar[t_next]).to(device=device, dtype=dtype)
            sqrt_one_minus_next = torch.sqrt(1.0 - diffusion.alpha_bar[t_next]).to(device=device, dtype=dtype)
            x_target_t = (sqrt_ab_next * x0_pred + sqrt_one_minus_next * eps_pred) * target_mask

        x_clean = x_clean * (1.0 - target_mask) + x_target_t * target_mask

    return x_clean * full_mask
...
def run_inference(
    ...
    if _is_hierarchical_frequency_model(inner) and not is_manifold_domain:
        if spec_mask is None:
            spec_mask = torch.ones_like(input_noise)
        x_t = hierarchical_denoise_ode(
            diffusion,
            cfg_model,
            input_noise,
            native_coords,
            ...
            norm_temps,
            mask,
            band_edges=tuple(int(edge) for edge in inner.band_edges),
            ordering=getattr(inner, "frequency_ordering", "causal"),
            top_k_freqs=K,
...
def train_step(
    ...
    is_time_domain = getattr(real_model, "is_time_domain", False)
    is_manifold_domain = getattr(real_model, "is_manifold_domain", False)
    hierarchy_mask_active = 0.0
    hierarchy_step_index = -1.0
    hierarchy_observed_count = 0.0
    hierarchy_target_count = 0.0
    hierarchy_target_frac = 1.0
    hierarchy_target_group_min = -1.0
    hierarchy_target_group_max = -1.0
    ...
    with dtype_ctx:
        ...
        # 3. Residue/feature mask plus optional hierarchical frequency-target mask.
        # The base mask protects invalid residues/features in both the noisy
        # input and the loss. Hierarchical masked diffusion then narrows only
        # the loss to the sampled target frequency group(s), while the model
        # input is masked separately through freq_keep_groups.
        if is_manifold_domain:
        ...
        freq_keep_groups = None
        if (
            not is_validation
            and not is_time_domain
            and not is_manifold_domain
            and hasattr(inner_model, "band_edges")
            and hasattr(inner_model, "num_frequency_groups")
        ):
            mask_prob = float(getattr(inner_model, "mask_training_prob", 0.0) or 0.0)
            num_frequency_groups = int(getattr(inner_model, "num_frequency_groups"))
            if mask_prob > 0.0 and torch.rand((), device=device).item() < mask_prob:
                selection = sample_frequency_mask_selection(
                    num_frequency_groups,
                    getattr(inner_model, "mask_training_ordering", getattr(inner_model, "frequency_ordering", "causal")),
                    mode=getattr(inner_model, "mask_training_mode", "next"),
                )
                complex_mult = 1 if is_dct else 2
                channels_per_mode = channels * complex_mult
                target_freq_mask = frequency_channel_mask(
                    top_k_freqs,
                    channels_per_mode,
                    getattr(inner_model, "band_edges"),
                    keep_groups=selection.target_groups,
                    device=device,
                    dtype=x_0.dtype,
                ).view(1, 1, -1)
                if target_freq_mask.shape[-1] != x_0.shape[-1]:
                    raise RuntimeError(
                        "Hierarchical target mask shape mismatch: "
                        f"mask D={target_freq_mask.shape[-1]} x_0 D={x_0.shape[-1]}"
                    )
                full_loss_mask = base_loss_mask * target_freq_mask
                freq_keep_groups = selection.observed_groups
                hierarchy_mask_active = 1.0
                hierarchy_step_index = float(selection.step_index)
                hierarchy_observed_count = float(len(selection.observed_groups))
                hierarchy_target_count = float(len(selection.target_groups))
                hierarchy_target_frac = float(target_freq_mask.mean().item())
                if selection.target_groups:
                    hierarchy_target_group_min = float(min(selection.target_groups))
                    hierarchy_target_group_max = float(max(selection.target_groups))
            elif mask_prob > 0.0:
                # Pass an explicit no-op keep mask so the Hierarchical model does
                # not also sample its legacy model-local mask independently.
                freq_keep_groups = tuple(range(num_frequency_groups))
        ...
        if freq_keep_groups is not None:
            model_batch["freq_keep_groups"] = freq_keep_groups
        if hierarchy_mask_active > 0.0:
            model_batch["freq_target_groups"] = selection.target_groups
        ...
        if not is_time_domain and not is_manifold_domain and prediction_target == "x_0":
            with torch.no_grad():
                ...
                low_idx = _low_k_group_index(band_edges)
                log_low_group = (
                    hierarchy_mask_active == 0.0
                    or (
                        hierarchy_target_group_min <= float(low_idx)
                        and hierarchy_target_group_max >= float(low_idx)
                    )
                )
                if log_low_group:
    ...
    metrics = {
        ...
        "debug/hierarchy_mask_active": hierarchy_mask_active,
        "debug/hierarchy_step_index": hierarchy_step_index,
        "debug/hierarchy_observed_count": hierarchy_observed_count,
        "debug/hierarchy_target_count": hierarchy_target_count,
        "debug/hierarchy_target_frac": hierarchy_target_frac,
        "debug/hierarchy_target_group_min": hierarchy_target_group_min,
        "debug/hierarchy_target_group_max": hierarchy_target_group_max,
    }
...
def TRAIN_DISTRIBUTED(
    ...
    frequency_ordering="causal", # hierarchical: causal | outside_in/bimodal | independent | bidirectional
    mask_training_prob=0.0, # hierarchical: optional input frequency-group masking probability
    mask_training_ordering=None, # hierarchical: masking order; defaults to frequency_ordering
    mask_training_mode="next", # hierarchical: next | suffix target-group schedule
    use_target_group_conditioning=True, # hierarchical: add learned target-frequency-group condition
    ...
    if use_zarr and os.path.exists(mdcath_zarr_path):
        ...
        heavy_hierarchy = str(model_type).lower() in {"hierarchical", "cascade"}
        default_num_workers = 4 if heavy_hierarchy else 16
        ...
        heavy_hierarchy = str(model_type).lower() in {"hierarchical", "cascade"}
        default_num_workers = 4 if heavy_hierarchy else 6
        num_workers = default_num_workers if dataloader_num_workers is None else int(dataloader_num_workers)
        if rank == 0:
            print(
    ...
    heavy_hierarchy = str(model_type).lower() in {"hierarchical", "cascade"}
    default_timeout = 300 if heavy_hierarchy else 60
    default_prefetch = 2 if heavy_hierarchy else 4
    ...
    if is_v17_model_type and prediction_target != "x_0":
        raise ValueError("v17a/v17b currently require prediction_target='x_0'")
    ...
    _configs = {
        ...
        "hierarchical": HierarchicalSpectralConfig(
            in_channels=total_channels, cond_channels=coord_channels,
            depth=num_layers, num_heads=num_heads,
            top_k_freqs=top_k_freqs, freq_hidden_size=freq_hidden_size,
            spectral_modes=effective_spectral_modes,
            ...
            ss_embed_dim=ss_embed_dim,
            band_edges=fast_band_edges,
            frequency_ordering=frequency_ordering,
            mask_training_prob=mask_training_prob,
            mask_training_ordering=mask_training_ordering,
            mask_training_mode=mask_training_mode,
            use_target_group_conditioning=use_target_group_conditioning,
        ),
...
if __name__ == "__main__":
    ...
    # Model
    parser.add_argument("--model_type", type=str, default=None, choices=["spectral_dit", "spectral_dit_low_k", "spectral_conv_dit", "spectral_conv_slow_branch", "spectral_conv_block_mix", "hierarchical", "cascade", "spectral_conv_block_mix_amplitude", "spectral_conv_block_mix_slow_hybrid", "dual_branch", "fno", "fno_manifold", "hno", "fno2", "fno2_bishop", "v14a", "v14b", "v14c", "v15", "v16", "v12c", "v17a", "v17b", "v12a_egnn", "v12b_egnn"])
    ...
    parser.add_argument("--frequency_ordering", type=str, default=None,
                        choices=["independent", "block", "causal", "low_to_high", "reverse_causal", "high_to_low", "outside_in", "bimodal", "bidirectional"],
                        help="hierarchical: frequency-group dependency order for the spectral mixer.")
    parser.add_argument("--mask_training_prob", type=float, default=None,
                        help="hierarchical: probability of masking input frequency groups during training.")
    parser.add_argument("--mask_training_ordering", type=str, default=None,
                        choices=["independent", "block", "causal", "low_to_high", "reverse_causal", "high_to_low", "outside_in", "bimodal", "bidirectional"],
                        help="hierarchical: ordering used when sampling input frequency masks.")
    parser.add_argument("--mask_training_mode", type=str, default=None,
                        choices=["next", "suffix"],
                        help="hierarchical: target schedule implied by sampled visible frequency groups.")
    parser.add_argument("--use_target_group_conditioning", action=argparse.BooleanOptionalAction, default=None,
                        help="hierarchical: add a learned target-frequency-group embedding to denoiser conditioning.")
    ...
    parser.add_argument("--aniso_legacy_direction", action=argparse.BooleanOptionalAction, default=None,
                        help="Use the pre-v12 high-k-heavy anisotropic direction.")
    parser.add_argument("--noise_schedule", type=str, default=None,
                        help="High-level noise preset, e.g. v12_current, cosine_low_first_targeted.")
