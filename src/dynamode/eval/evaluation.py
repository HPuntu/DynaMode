...
def build_model_stack(config: dict, device: torch.device):
    ...
    representation_name = canonical_representation(
        ...
        displacement=config.get("displacement", True),
    )
    representation_obj = CoordinateRepresentation(
        representation_name,
        coord_channels=coord_channels,
        length_min=config.get("representation_length_min", 3.5),
        length_max=config.get("representation_length_max", 4.1),
        length_residual_max=config.get("representation_length_residual_max", 0.30),
    )
    if representation_obj.is_unit_chain and coords_type != "ca":
        raise ValueError(f"{representation_obj.name} requires coords_type='ca'")
    repr_coord_channels = representation_obj.model_coord_channels
    ...
    log(format_noise_diagnostics(resolved_noise.diagnostics))

    diffusion = SpectralDiffusion(
        T=config.get("num_steps", 1000), device=device,
    ...
    return {
        "model": model, "diffusion": diffusion, "transform_engine": transform_engine,
        "coord_channels": coord_channels, "repr_coord_channels": repr_coord_channels,
        "angle_channels": angle_channels, "total_channels": total_channels, "window_size": window_size,
        "top_k_freqs": top_k_freqs,
