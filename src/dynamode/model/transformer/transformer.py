...
from dynamode.spectral.adapters import SpectralAdapter, DCT, DFT
from dynamode.spectral.conditioned_freq_scale import size_bin_label
...
from dynamode.model.spec_conv.spectral_conv import _parse_low_k_correction_spec
...
class SpectralDiT(nn.Module):
    ...
    Input
    ----- 
    raw DCT coefficients of coordinate deviations (x - native), shape (B, L, K*C_in).
    ...
    stay in raw coefficient scale. Each residue token = flattened (K, C_in) coefficients 
    projected per-frequency to (K, freq_hidden_size) then flattened to D = K * freq_hidden_size.
